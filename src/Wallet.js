...
export default function PermanentDrawerLeft() {
  ...
  const {
    ...
    error,
    isValidating,
  } = useSWR('http://localhost:3000/get/totalwallets', fetcher, { refreshInterval: 36000000 });
  ...
  async function blockuser(shop) {
    const urlencoded = new URLSearchParams()
    urlencoded.append("shop", shop)
      return fetch('http://localhost:3000/user/block', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded'
        },
  ...
  async function updaterole(shop, role) {
    ...
    urlencoded.append("role", role)
      return fetch('http://localhost:3000/user/addrole', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded'
        },
